...
#import numpy.trace as np.trace
...
def shape(X, i=-1):
    if i==-1 : return np.shape(X)
    else     : return np.shape(X)[i]
...
class Anova(OrderedDict):
    ...
    def run(self, dataframe, dv, wfactors=None, bfactors=None,
        ...
        if   transform in ['log','log10']:
            self.transform=np.log10
            tstr='LOG_'
            
        elif transform in ['reciprocal','inverse']:
            self.transform=lambda X:1./np.array(X)
            tstr='RECIPROCAL_'
            
        elif transform in ['square-root','sqrt']:
            self.transform=np.sqrt
            tstr='SQRT_'
            
        elif transform in ['arcsine','arcsin'] :
            self.transform=np.arcsin
            tstr='ARCSIN_'
            
        elif transform in ['windsor01']:
            self.transform=lambda X: windsor(np.array(X),.01)
            tstr='WINDSOR01_'
            
        elif transform in ['windsor05']:
            self.transform=lambda X: windsor(np.array(X),.05)
            tstr='WINDSOR05_'
            
        elif transform in ['windsor10']:
            self.transform=lambda X: windsor(np.array(X),.10)
            tstr='WINDSOR10_'        
...
##                print('%i degrees of freedom lost from trim'%int(self.dftrim))
        ...
        self.pt_asarray = np.array(self.pt, dtype=np.float64)

##        print('%i NaN values found.'%len(self.pt_asarray[isnan(self.pt_asarray)]))

        # Replace NaN values with mean of dv
        self.pt_asarray[np.isnan(self.pt_asarray)]=np.mean(self.df[self.dv])
    ...
    def _between(self):
        ...
        if shape(pt_asarray,1) != Nd:
            ...
            raise Exception('data has {:} conditions; design only {:}'.format(
                            shape(pt_asarray,1),Nd))
        ...
        for f in xrange(1,Nf+1):
            ...
            sc[(f,2)] = detrend(np.eye(D[f-1]),type='constant')
        ...
        for i in xrange(1,len(factors)+1):
            for efs in _xunique_combinations(factors, i):
                ...
                r['obs'] /= np.prod([len(conditions[f])*1. for f in efs])
                ...
                dstd = np.std(self.df[self.dv])
                dmu  = np.mean(self.df[self.dv])
    ...
    def _mixed(self):
        ...
        for e in xrange(1,Ne+1):
            ...
            r['y2'] = mean(np.dot(pt_asarray,cy),0)
            
            # df for effect
            r['df'] =  np.prod([len(conditions[f])-1. for f in efs])
            ...
            # Sphericity assumed
            r['ss']  = np.sum(y*b.T)
            r['ss'] /= No/(np.prod([len(conditions[f]) for f in bfactors])*1.)
            r['mss'] = r['ss']/r['df']
        ...
        ss_bsub *= (np.prod([len(conditions[f]) for f in wfactors])*1.)

        df_b  = np.prod([len(conditions[f]) for f in bfactors])
        df_b *= Nr/np.prod([len(conditions[f]) for f in bfactors])
        df_b -= 1.

        dfe_b  =  np.prod([len(conditions[f]) for f in bfactors])
        dfe_b *= (Nr/np.prod([len(conditions[f]) for f in bfactors])-1.)
        ...
        self[('TOTAL',)] = {'ss' : ss_total,
                            'df' : Nr/np.prod([len(conditions[f])*1.
        ...
        for i in xrange(1, len(wfactors)+1):
            for efs in _xunique_combinations(wfactors, i):
                ...
                r={}
                tmp=np.array(df.pivot(self.dv, cols=efs[:-1], rows=[self.sub]),
                          dtype=np.float64).flatten()
                r['ss']  = np.sum((tmp-np.mean(pt_asarray))**2)
                r['ss'] *= np.prod([len(conditions[f]) for f in wfactors
                                 if f not in efs])
                ...
                r['df']  = np.prod([len(conditions[f]) for f in bfactors])
                r['df'] *= np.prod([len(conditions[f])-1. for f in efs \
                                 if f in wfactors])
                r['df'] *= Nr/np.prod([len(conditions[f])*1. for f in bfactors])-1.
                dfe_sum += r['df']
        ...
        for i in xrange(1,len(factors)+1):
            for efs in _xunique_combinations(factors, i):
                ...
                if efs not in self.befs:
                    ...
                    # calculate observations per cell
                    r['obs'] = Nr/np.prod([len(conditions[f])*1. for f in bfactors])
                    r['obs'] *= np.prod([len(conditions[f])*1. for f in factors])
                    r['obs'] /= np.prod([len(conditions[f])*1. for f in efs])
                    ...
                    # calculate non-centrality and observed power
                    dave=np.array(df.pivot(self.dv,cols=efs),
                               dtype=np.float64).flatten()
                    dstd=np.std(self.df[self.dv])
                    dmu  = np.mean(self.df[self.dv])
                    ...
                    for x in ['_gg','_hf','_lb']:
                        r['df{}'.format(x)] = r['df']*r['eps{}'.format(x)]
                        r['dfe{}'.format(x)] = r['dfe']*r['eps{}'.format(x)]
                        r['mss{}'.format(x)] = r['ss']/r['df{}'.format(x)]
                        r['mse{}'.format(x)] = r['sse']/r['dfe{}'.format(x)]
                        r['F{}'.format(x)] = r['mss{}'.format(x)]/r['mse{}'.format(x)]
                        r['p{}'.format(x)] = scipy.stats.f(r['df{}'.format(x)],r['dfe{}'.format(x)]).sf(r['F{}'.format(x)])
                        r['obs{}'.format(x)] = r['obs']
                        r['critT{}'.format(x)] = abs(scipy.stats.t(r['dfe']).ppf(.05/2.))
                        r['se{}'.format(x)] = np.sqrt(r['mse']/r['obs{}'.format(x)])*\
                                         r['critT{}'.format(x)]/1.96
                        r['ci{}'.format(x)] = np.sqrt(r['mse']/r['obs{}'.format(x)])*\
                                         r['critT{}'.format(x)]

                        # calculate non-centrality and observed power
                        r['lambda{}'.format(x)]=r['lambda']
                        r['power{}'.format(x)]=observed_power( r['df'], r['dfe'], r['lambda'],
                                                       eps=r['eps{}'.format(x)])
    ...
    def _within(self):
        ...
        for i in xrange(1,len(factors)+1):
            for efs in _xunique_combinations(factors, i):
                ...
                r['df']  = np.prod([len(conditions[f])-1 for f in factors if \
                                 f in efs])
        ...
        for e in xrange(1,Ne+1):
            ...
            for x in ['_gg','_hf','_lb']:
                ...
                r['F{}'.format(x)] = r['mss{}'.format(x)]/r['mse{}'.format(x)]
                r['p{}'.format(x)] = scipy.stats.f(r['df{}'.format(x)],r['dfe{}'.format(x)]).sf(r['F{}'.format(x)])
                r['obs{}'.format(x)] = Nr*No
                r['critT{}'.format(x)] = abs(scipy.stats.t(r['dfe']).ppf(.05/2.))
                r['se{}'.format(x)] = np.sqrt(r['mse']/r['obs{}'.format(x)])*r['critT{}'.format(x)]/1.96
                r['ci{}'.format(x)] = np.sqrt(r['mse']/r['obs{}'.format(x)])*r['critT{}'.format(x)]

                # calculate non-centrality and observed power
                r['lambda{}'.format(x)]=r['lambda']
                r['power{}'.format(x)]=observed_power( r['df'], r['dfe'], r['lambda'] ,eps=r['eps{}'.format(x)])
        ...
        ss_subject *= (np.prod([len(conditions[f]) for f in factors])*1.)
...
##        self.__html__()
    ...
    def _between_html(self, html):
        ...
        if self.measure=='':
            ...
            html.add(h('Measure: {}'.format(self.dv)))

        else:
            #html.add(h('Measure: %s of %s'%(self.dv,self.measure)))
            html.add(h('Measure: {} of {}'.format(self.dv,self.measure)))
    ...
    def _summary_html(self, html, factors):
        ...
        for i in xrange(1,len(factors)+1):
            for efs in _xunique_combinations(factors, i):
                ...
                dave=np.array([   np.mean(y[0]) for y in ys ])
                dsem=np.array([ stderr(y[0]) for y in ys ])
...
##        return ''.join(s)
    ...
    def __str__(self):
        if self.measure == '':
            #title  = '%s ~'%self.dv
            title  = '{} ~'.format(self.dv)
        else:
            #title  = '%s of %s ~'%(measure, self.dv)
            title  = '{} of {} ~'.format(measure, self.dv)
    ...
    def _summary_str(self, factors):
        ...
        for i in xrange(1,len(factors)+1):
            for efs in _xunique_combinations(factors, i):
                ...
                dave=np.array([   np.mean(y.flatten()) for y in ys ])
                dsem=np.array([ stderr(y.flatten()) for y in ys ])
...
##
            ...
            txt=['Summary Plot of {} ~ {}'.format(val, xaxis)]
            #if seplines!=''  : txt.append(' * %s'%seplines)
            #if sepxplots!='' : txt.append(' * %s'%sepxplots)
            #if sepyplots!='' : txt.append(' * %s'%sepyplots)
            if seplines!=''  : txt.append(' * {}'.format(seplines))
            if sepxplots!='' : txt.append(' * {}'.format(sepxplots))
            if sepyplots!='' : txt.append(' * {}'.format(sepyplots))
            txt=''.join(txt)
...
##            html.add(a(img(fname,width=int(width),height=int(height)),href=fname))
    ...
    def __repr__(self):
        ...
        if self.df != {}:
            ...
            kwds.append(", df={}".format(repr(self.df)))
            
        if self.wfactors != []:
            #kwds.append(", wfactors=%s"%repr(self.wfactors))
            kwds.append(", wfactors={}".format(repr(self.wfactors)))
                
        if self.bfactors != []:
            #kwds.append(", bfactors=%s"%repr(self.bfactors))
            kwds.append(", bfactors={}".format(repr(self.bfactors)))

        if self.alpha != 0.05:
            #kwds.append(", alpha=%s"%self.alpha)
            kwds.append(", alpha={}".format(self.alpha))

        if self.measure != '':
            #kwds.append(", measure='%s'"%self.measure)
            kwds.append(", measure='{}'".format(self.measure))

        if self.dv != None:
            #kwds.append(", dv='%s'"%self.dv)
            kwds.append(", dv='{}'".format(self.dv))
            
        if self.sub != 'SUBJECT':
            #kwds.append(", sub='%s'"%self.sub)
            kwds.append(", sub='{}'".format(self.sub))

        if self.transform != '':
            #kwds.append(", transform='%s'"%self.transform)
            kwds.append(", transform='{}'".format(self.transform))
        
        kwds= ''.join(kwds)
        
        return 'Anova({}{})'.format(args,kwds)
